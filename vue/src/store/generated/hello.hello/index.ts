import { Client, registry, MissingWalletError } from 'hello-client-ts'

import { Params } from "hello-client-ts/hello.hello/types"
import { Post } from "hello-client-ts/hello.hello/types"
	...
	actions: {
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.HelloHello.query.queryParams()).data
		...
		async QueryHello({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.HelloHello.query.queryHello()).data
		...
		async QueryPosts({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.HelloHello.query.queryPosts(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.HelloHello.query.queryPosts({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgCreatePost({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.HelloHello.tx.sendMsgCreatePost({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgCreatePost({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.HelloHello.tx.msgCreatePost({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreatePost:Init Could not initialize signing client. Wallet is required.')
